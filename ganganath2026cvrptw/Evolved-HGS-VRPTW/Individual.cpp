...
void Individual::evaluateCompleteCost()
{
	...
	// Route penalty biases search toward fewer vehicles (NV contributes 1000x to metric)
	myCostSol.penalizedCost = myCostSol.distance + myCostSol.nbRoutes * 200.0 + myCostSol.capacityExcess * params->penaltyCapacity + myCostSol.timeWarp * params->penaltyTimeWarp + myCostSol.waitTime * params->penaltyWaitTime;
...
void Individual::exportCVRPLibFormat(std::string fileName)
{
	...
	if (myfile.is_open())
	{
		...
		myfile << "Cost " << (int) myCostSol.distance << std::endl;
...
void Individual::printCVRPLibFormat()
{
	...
	std::cout << "Cost " << (int) myCostSol.distance << std::endl;
