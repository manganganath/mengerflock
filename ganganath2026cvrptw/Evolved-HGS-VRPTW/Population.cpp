...
bool Population::addIndividual(const Individual* indiv, bool updateFeasible)
{
	...
	for (Individual* myIndividual2 : subpop)
	{
		...
		if (myDistance < minDist) minDist = myDistance;
	...
	// Diversity gate: reject near-duplicate individuals when the population is already at minimum size.
	// This prevents population collapse under strong tournament selection pressure.
	if (static_cast<int>(subpop.size()) >= params->config.minimumPopulationSize && minDist < 0.02)
	{
		for (Individual* myIndividual2 : subpop)
			myIndividual2->removeProximity(myIndividual);
		delete myIndividual;
		return false;
	}
...
void Population::managePenalties()
{
	...
	for (int i = 0; i < static_cast<int>(infeasibleSubpopulation.size()); i++)
	{
		infeasibleSubpopulation[i]->myCostSol.penalizedCost = infeasibleSubpopulation[i]->myCostSol.distance
			+ infeasibleSubpopulation[i]->myCostSol.nbRoutes * 200.0
			...
			+ params->penaltyWaitTime * infeasibleSubpopulation[i]->myCostSol.waitTime;
	}
...
{
	int nTotal = (int)(feasibleSubpopulation.size() + infeasibleSubpopulation.size());
	if (nTotal == 0) return nullptr;

	// Tournament of size 5 from the full combined pool — stronger selection pressure
	// than binary tournament while preserving feasible/infeasible balance
	int tournSize = std::min(5, nTotal);
	Individual* best = nullptr;
	for (int i = 0; i < tournSize; i++)
	{
		int idx = params->rng() % nTotal;
		Individual* cand = (idx < (int)feasibleSubpopulation.size())
		    ? feasibleSubpopulation[idx]
		    : infeasibleSubpopulation[idx - (int)feasibleSubpopulation.size()];
		if (!best || cand->biasedFitness < best->biasedFitness)
			best = cand;
	}
	return best;
}
...
{
	// Update the fitness values once before all tournament selections
