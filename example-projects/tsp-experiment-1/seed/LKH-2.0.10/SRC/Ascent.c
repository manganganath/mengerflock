    ...
    for (Period = InitialPeriod, T = InitialStepSize * Precision;
        ...
        for (P = 1; T && P <= Period && Norm != 0; P++) {
            /* Adjust the Pi-values with adaptive momentum:
               when V and LastV have opposite signs (oscillating), use 50/50
               mix for more damping; when same sign, use 90/10 for faster
               convergence; default 70/30 when LastV is zero */
            t = FirstNode;
            do {
                if (t->V != 0) {
                    int wV, wL;
                    if (t->LastV != 0) {
                        if ((t->V > 0) == (t->LastV > 0)) {
                            wV = 9; wL = 1; /* same sign: responsive */
                        } else {
                            wV = 5; wL = 5; /* opposite sign: damp oscillation */
                        }
                    } else {
                        wV = 7; wL = 3; /* LastV=0: original weights */
                    }
